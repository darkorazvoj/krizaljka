    ...
    KrizaljkaSlot neighborSlot)
{
    ...
    private IReadOnlyList<Term> CollapseToDistinctAvailableTerms(IEnumerable<Term> terms)
{
    List<Term> result = [];
...
private IReadOnlyList<Term> RefreshCollapsedDomainTerms(IReadOnlyList<Term> collapsedTerms)
{
    List<Term> result = [];
